...
class DataManager {
    ...
    func addMovieData() {
        if let randomMovie = movieDataArray.randomElement() {
            movieDataArray.append(randomMovie)
        }
        
        if movieDataArray.isEmpty {
            let movie = Movie(movieImage: UIImage(named: "spiderman2.png"), movieName: "스파이더맨2", movieDescription: "추가된 객체")
            movieDataArray.append(movie)}
    }

    func popMovieData() {
        if !movieDataArray.isEmpty { movieDataArray.remove(at: movieDataArray.count - 1) }
        else { self.makeMovieData()}
    }
}
